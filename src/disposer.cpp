...
namespace disposer{
	...
	void disposer::add_module_maker(
	...
	class disposer::impl{
	public:
		std::unordered_map< std::string, disposer::maker_function >& maker_list_;
	...
	module_ptr disposer::impl::make_module(make_data&& data){
	...
	auto disposer::impl::create_chains(types::merge::config&& config){
		...
		std::unordered_map<
			std::string,
			std::vector< std::reference_wrapper< chain > >
		> groups;
		...
		for(auto& config_chain: config.chains){
			log([&config_chain](log_base& os){
				...
				std::map<
					std::string,
					std::pair< output_base&, bool >
				> variables;

				// create modules
				for(
					std::size_t i = 0;
					i < config_chain.modules.size();
					++i
				){
				...
				// go through all modules and activate output typs in the
				// target inputs
				...
				for(auto& config_module: config_chain.modules){
					...
					log([&config_module](log_base& os){
						...
						// names
						for(auto& config_input: config_module.inputs){
							auto output_iter = variables.find(
							...
							// try to activate the types from output in
							// input
	...
	void disposer::load(std::string const& filename){
		auto config = log([&filename](log_base& os){
			os << "parse '" << filename << "'";
		}, [&filename](){
		...
		auto merged_config = log([](log_base& os){
			os << "merge";
		}, [&config](){
		...
		log([](log_base& os){
			...
			std::tie(chains_, id_generators_, groups_) =
				disposer::impl{maker_list_}.create_chains(std::move(merged_config));
		});
	}
